...
class POCEngine:
    ...
    def __init__(self, timeout: float = 10.0, concurrency: int = 10,
        ...
        if stealth:
            from src.stealth import StealthClient
    ...
    async def scan_target(self, target_url: str,
                          ...
                          tags: List[str] = None,
                          severity: List[str] = None) -> List[MatchResult]:
        """
        用所有模板扫描单个目标
    ...
    async def scan_targets(self, targets: List[str],
        ...
        async with httpx.AsyncClient(
            ...
            async def _scan_one(target: str):
                async with sem:
                    return target, await self.scan_target(target, templates, tags, severity)
    ...
    async def _execute_template(self, client: httpx.AsyncClient,
        ...
        for req in tmpl.requests:
            try:
                result = await self._execute_request(client, target_url, tmpl, req, variables)
    ...
    async def _execute_request(self, client: httpx.AsyncClient,
                               ...
                               tmpl: Template,
                               req: HTTPRequest,
                               variables: Dict[str, str]) -> Optional[MatchResult]:
        """执行单个 HTTP 请求"""
        # 构建请求 URL
        urls = req.path if req.path else [f"{target_url.rstrip('/')}"]
        url = self._expand_variables(urls[0], variables)
        ...
        try:
            ...
            if self._stealth_client:
                if req.method == "GET":
                    resp = await self._stealth_client.get(url, headers=headers, timeout=req.timeout)
                elif req.method == "POST":
                    resp = await self._stealth_client.post(url, headers=headers, data=body, timeout=req.timeout)
                elif req.method == "PUT":
                    resp = await self._stealth_client.request("PUT", url, headers=headers, data=body, timeout=req.timeout)
                elif req.method == "DELETE":
                    resp = await self._stealth_client.request("DELETE", url, headers=headers, timeout=req.timeout)
                elif req.method == "HEAD":
                    resp = await self._stealth_client.request("HEAD", url, headers=headers, timeout=req.timeout)
                elif req.method == "OPTIONS":
                    resp = await self._stealth_client.request("OPTIONS", url, headers=headers, timeout=req.timeout)
                elif req.method == "PATCH":
                    resp = await self._stealth_client.request("PATCH", url, headers=headers, data=body, timeout=req.timeout)
                elif req.method == "TRACE":
                    resp = await self._stealth_client.request("TRACE", url, headers=headers, timeout=req.timeout)
                else:
                    resp = await self._stealth_client.request(req.method, url, headers=headers, data=body, timeout=req.timeout)
            elif req.method == "GET":
                resp = await client.get(url, headers=headers, timeout=req.timeout)
            elif req.method == "POST":
                resp = await client.post(url, headers=headers, content=body, timeout=req.timeout)
            elif req.method == "PUT":
                resp = await client.put(url, headers=headers, content=body, timeout=req.timeout)
            elif req.method == "DELETE":
                resp = await client.delete(url, headers=headers, timeout=req.timeout)
            elif req.method == "HEAD":
                resp = await client.head(url, headers=headers, timeout=req.timeout)
            elif req.method == "OPTIONS":
                resp = await client.options(url, headers=headers, timeout=req.timeout)
            elif req.method == "PATCH":
                resp = await client.patch(url, headers=headers, content=body, timeout=req.timeout)
            else:
                resp = await client.request(req.method, url, headers=headers, content=body, timeout=req.timeout)
